...
public class PassController2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		/*
		/mvcboard/pass.do?mode=delete&idx=152
		...
		req.getRequestDispatcher("/Freeboard/Pass2.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(confirmed) {
			...
			if(mode.equals("edit")) {
				...
				session영역은 페이지를 이동해도 데이터가 공유되므로 해당 게시뭏을
				수정완료할때까지는 유지하고, 수정이 완료되면 제거할 것이다.
				여기에서 저장한 패스워드는 수정을 위한 update쿼리문의 
				where절 조건으로 사용된다.
				 */
				HttpSession session = req.getSession();
				session.setAttribute("pass", pass);
				resp.sendRedirect("../Home/edit2.do?idx="+ idx);
			}
			else if(mode.equals("delete")) {
				//삭제인 경우 커넥션풀을 통해 DB연결
				dao = new MVCBoardDAO2();
				//기존의 게시물을 얻어온다.
				MVCBoardDTO2 dto = dao.selectView(idx);
				//게시물을 삭제한다.
				...
				JSFunction.alertLocation(resp, "삭제되었습니다.", "../Home/list2.do");
			}
			else {
