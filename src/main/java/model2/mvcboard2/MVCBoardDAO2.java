...
public class MVCBoardDAO2 extends DBConnPool{
   ...
   //부모클래스의 기본생성자 호출을 통해 DB를 연결한다.
    ...
    public int selectCount(Map<String, Object> map) {
        int totalCount = 0;
        String query = "SELECT COUNT(*) FROM mvcboard2 ";
     ...
     */
    public List<MVCBoardDTO2> selectListPage(Map<String, Object> map) { 
        List<MVCBoardDTO2> board = new ArrayList<MVCBoardDTO2>();  
        String query = " "
              + " SELECT * FROM ( "
    ...
    public int insertWrite(MVCBoardDTO2 dto) {
       int result = 0;
       try {
          /*
          컬럼 설명
           ...
           */
            String query = "INSERT INTO mvcboard2 ( "
    ...
    public MVCBoardDTO2 selectView(String idx) {
       
       //조회된 레코드를 DTO객체 저장한 후 반환한다.
       MVCBoardDTO2 dto = new MVCBoardDTO2();
       //인파라미터가 있는 쿼리문 작성
       String query = "SELECT * FROM mvcboard2 WHERE idx=?";
       try {
    ...
    public void updateVisitCount(String idx) {
       String query ="UPDATE mvcboard2 SET "
    ...
    public void downCountPlus(String idx) {
       String sql = "UPDATE mvcboard2 SET "
  ...
  //수정/삭제전 해당 게시물의 패스워드를 검증한다.
  	...
  	public boolean confirmPassword(String pass, String idx) {
  		boolean isCorr = true;
  		try {
  			String sql = "SELECT COUNT(*) FROM mvcboard2 WHERE pass=? AND idx=?";
  			psmt = con.prepareStatement(sql);
  	...
  	public int deletePost(String idx) {
  		int result = 0;
  		try {
  			String query = "DELETE FROM mvcboard2 WHERE idx=?";
  			psmt = con.prepareStatement(query);
  			psmt.setString(1, idx);
  			result= psmt.executeUpdate();
  		}
  	...
  	public int updatePost(MVCBoardDTO2 dto) {
  		int result = 0;
  		
  		try {
  			//update쿼리문 작성
  			String query = "UPDATE mvcboard2 "
