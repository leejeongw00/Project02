...
public class ListController2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		특정 페이지로 진입한 경우에는 파라미터를 받아서 구산을 계산해서 얻어옴.
		 */
		int pageNum = 1;
		...
		List<MVCBoardDTO2> boardLists = dao.selectListPage(map);
		//커넥션풀에 자원 반납
		dao.close();
		
		//페이지 번호를 생성하기 위한 유틸리티 클래스의 메서드 호출
		//모델1 방식의 게시판에서 사용했던 메서드를 그대로 사용한다.
		String pagingImg = BoardPage.pagingStr(totalCount, pageSize, blockPage, pageNum, "../Home/list2.do");
		...
		//View로 포워드를 걸어준다.
		req.getRequestDispatcher("/Freeboard/List2.jsp").forward(req, resp);
	}
